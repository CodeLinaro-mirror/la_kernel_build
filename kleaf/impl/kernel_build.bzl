...
def _gen_symvers_step(ctx, all_output_names_minus_modules, kbuild_mixed_tree_ret):
    ...
    # For mixed builds, if modules in MAKE_GOALS, we are building in-tree modules against
    # base_kernel.
    # After 6.13, in this case, Kbuild only generates modules-only.symvers, and Module.symvers is no
    # longer generated by Kbuild. Manually
    # concatenate modules-only.symvers with vmlinux.symvers to form Module.symvers.
    if "Module.symvers" in all_output_names_minus_modules and kbuild_mixed_tree_ret.base_kernel_files:  # is mixed build
        ...
        # <(:) ensures that cat exits if there are no symvers_srcs
        cmd += """
            cat {symvers_srcs} <(:) > ${{OUT_DIR}}/Module.symvers
            if grep -q "\\bmodules\\b" <<< "{make_goals}"; then
                ...
                cat ${{OUT_DIR}}/modules-only.symvers >> ${{OUT_DIR}}/Module.symvers
            fi
        """.format(
